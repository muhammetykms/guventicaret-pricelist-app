...
from ...repositories.antifriz import AntifrizRepository
...
class AntifrizTab:
    """
    Tablo:
      Marka | Urun | Litre | Birim Fiyat (TL) | Satış Fiyat (TL) | ID
    Çift tıklama: DETAY (sadece görüntü)
    """
    def __init__(self, container):
        self.repo = AntifrizRepository()
    ...
    def _build(self):
        ...
        cols = ("marka","urun","litre","birim_fiyat","satis_fiyat","id")
        headers = {
            "marka":"Marka",
            "urun":"Urun",
            "litre":"Litre",
        ...
        for i, c in enumerate(cols):
            ...
            self.tv.column(c, width=widths[i], anchor="w", stretch=False)
    ...
    def refresh(self):
        ...
        for row in self.repo.all(q):
            # repo.all beklenen: (marka, urun, litre, birim_fiyat, satis_fiyat, id)
            marka, urun, lt, bf, sf, pid = (list(row) + [None]*6)[:6]
            self.tv.insert("", "end", values=(marka or "", urun or "", "" if lt is None else lt, money(bf), money(sf), pid))
    ...
    def _show_detail(self):
        ...
        p = self.repo.get(pid)
        if not p: return
        # repo.get beklenen: (id, marka, urun, litre, birim_fiyat, satis_fiyat)
        _id, marka, urun, lt, bf, sf = p[:6]

        win = tk.Toplevel(self.container); win.title("Antifriz Detay"); win.configure(bg=THEME["BG"])
        fields = [
            ("Marka", marka),
            ("Urun", urun),
            ("Litre", "" if lt is None else str(lt)),
            ("Birim Fiyat", f"{money(bf)} TL"),
            ("Satış Fiyat", f"{money(sf)} TL"),
        ]
        ...
        ttk.Button(win, text="Kapat", style="Red.TButton", command=win.destroy).grid(row=len(fields),column=0,columnspan=2,pady=10)
    ...
    def _open_form(self, pid):
        win = tk.Toplevel(self.container); win.title("Antifriz " + ("Ekle" if pid is None else "Düzenle")); win.configure(bg=THEME['BG'])
        for c in (0,1): win.grid_columnconfigure(c, weight=1)

        vars = {k: tk.StringVar() for k in ["marka","urun","litre","birim_fiyat","satis_fiyat"]}
        ...
        row(1, "urun", "Urun")
        row(2, "litre", "Litre")
        e_unit = row(3, "birim_fiyat", "Birim Fiyat (TL)")
        e_sale = row(4, "satis_fiyat", "Satış Fiyat (TL)")

        # Otomatik satış fiyatı
        lf = ttk.LabelFrame(win, text="Satış fiyatını otomatik belirle", padding=10)
        lf.grid(row=5, column=0, columnspan=2, sticky="we", padx=10, pady=(4,10))
        ...
        tk.Label(lf, textvariable=preview, bg=THEME["BG"], fg=THEME.get("GREEN","#0a7a3b")).grid(row=0,column=1,sticky="e")
        ...
        if pid is not None:
            ...
            if p:
                _id, marka, urun, lt, bf, sf = p[:6]
                vars["marka"].set(marka or "")
                vars["urun"].set(urun or "")
                vars["litre"].set("" if lt is None else str(lt))
                vars["birim_fiyat"].set("" if bf is None else format_tr_money(bf))
                vars["satis_fiyat"].set("" if sf is None else format_tr_money(sf))
                recalc_preview()

        btns = tk.Frame(win, bg=THEME['BG']); btns.grid(row=6, column=0, columnspan=2, pady=10, sticky="e", padx=10)
    ...
    def _save(self, pid, vars, auto, win):
        try:
            marka = vars["marka"].get().strip() or None
            urun  = vars["urun"].get().strip()
            lt    = vars["litre"].get().strip()
            lt    = float(lt.replace(",", ".")) if lt else None

            unit = parse_tr_money(vars["birim_fiyat"].get())
            sale = parse_tr_money(vars["satis_fiyat"].get())
            if auto: sale = calc_sale_from_unit(unit, 0.20)

            data = {"marka": marka, "urun": urun, "litre": lt, "birim_fiyat": unit, "satis_fiyat": sale}
        except Exception as e:
            messagebox.showerror("Hata", f"Alanları kontrol edin:\n{e}"); return

        if not data["urun"]:
            messagebox.showerror("Hata", "Urun alanı zorunlu."); return
