...
class YagTab:
    """
    Tablo:
      Marka | Urun | Viskozite | Litre | Birim Fiyat (TL) | Satış Fiyat (TL) | ID
    Çift tıklama: DETAY (sadece görüntü)
    """
    def __init__(self, container):
        self.repo = YagRepository()
    ...
    def _build(self):
        ...
        cols = ("marka", "urun", "viskozite", "litre", "birim_fiyat", "satis_fiyat", "id")
        headers = {
            "marka": "Marka",
            "urun": "Urun",
            "viskozite": "Viskozite",
        ...
        widths = [180, 440, 140, 100, 160, 160, 60]
    ...
    def refresh(self):
        ...
        for r in self.repo.all(q):
            marka, urun, vis, lt, bf, sf, pid = (list(r) + [None]*7)[:7]
            self.tv.insert("", "end", values=(marka or "", urun or "", vis or "", lt or 0, money(bf), money(sf), pid))
    ...
    def _show_detail(self):
        ...
        p = self.repo.get(pid)
        if not p: return
        _id, marka, urun, vis, lt, bf, sf = p

        win = tk.Toplevel(self.container); win.title("Yağ Detay"); win.configure(bg=THEME["BG"])
        fields = [
            ("Marka", marka),
            ("Urun", urun),
            ("Viskozite", vis),
    ...
    def _open_form(self, pid):
        win = tk.Toplevel(self.container); win.title("Yağ " + ("Ekle" if pid is None else "Düzenle")); win.configure(bg=THEME['BG'])
        for c in (0,1): win.grid_columnconfigure(c, weight=1)

        vars = {k: tk.StringVar() for k in ["marka","urun","viskozite","litre","birim_fiyat","satis_fiyat"]}
        ...
        row(0, "marka", "Marka")
        row(1, "urun", "Urun")
        row(2, "viskozite", "Viskozite")
        row(3, "litre", "Litre")
        e_unit = row(4, "birim_fiyat", "Birim Fiyat (TL)")
        e_sale = row(5, "satis_fiyat", "Satış Fiyat (TL)")
        ...
        if pid is not None:
            _id, marka, urun, vis, lt, bf, sf = self.repo.get(pid)
            vars["marka"].set(marka or "")
            vars["urun"].set(urun or "")
            vars["viskozite"].set(vis or "")
    ...
    def _save(self, pid, vars, auto, win):
        try:
            ...
            vis   = vars["viskozite"].get().strip() or None
            ...
            data = {"marka": marka, "urun": urun, "viskozite": vis, "litre": lt, "birim_fiyat": unit, "satis_fiyat": sale}
